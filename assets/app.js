...
/* ============================================================
   CLOUDFLARE WARM-UP — evita il blocco al primo avvio di un contenuto
   VixSrc e protetto da Cloudflare. La primissima richiesta che parte
   da un contesto "freddo" (nessuna sessione ancora aperta su
   vixsrc.to dentro l'iframe del player) puo essere respinta dal WAF
   con la pagina "Sorry, you have been blocked". Appena si cambia
   episodio pero la richiesta successiva funziona sempre, perche non
   e piu la prima verso quel dominio nella stessa sessione del
   browser. Per evitare che l'utente veda il blocco proprio la prima
   volta, "scaldiamo" silenziosamente il dominio in un iframe
   invisibile non appena l'app parte, molto prima che l'utente prema
   play: cosi il primo caricamento visibile e sempre, di fatto, gia
   la seconda richiesta verso quel dominio in questa sessione.
   ============================================================ */
const CF_WARMUP_DOMAINS={vixsrc:'https://vixsrc.to/',['vixsrc-it']:'https://vixsrc.to/'};
const cfWarmupState={};
function isCloudflareGuardedSource(src){return !!CF_WARMUP_DOMAINS[src];}
function warmUpDomain(src){
  const url=CF_WARMUP_DOMAINS[src];
  if(!url)return Promise.resolve();
  if(cfWarmupState[src])return cfWarmupState[src];
  const frame=document.createElement('iframe');
  frame.setAttribute('aria-hidden','true');
  frame.setAttribute('tabindex','-1');
  frame.style.cssText='position:fixed;left:-9999px;top:-9999px;width:1px;height:1px;opacity:0;border:0;pointer-events:none';
...
async function ensureDomainWarmedUp(src){
  if(!CF_WARMUP_DOMAINS[src])return;
  try{await warmUpDomain(src);}catch(e){}
}
/* Avvia il riscaldamento il prima possibile, appena lo script parte */
if(document.body)warmUpDomain('vixsrc');
else document.addEventListener('DOMContentLoaded',()=>warmUpDomain('vixsrc'),{once:true});
...
async function setPlayerFrameSrc(id,type,season,episode,src,startSecs){
  ...
  const guarded=!anime&&isCloudflareGuardedSource(src);
  if(anime){
    setFrameMessage(fr,'Caricamento episodio','Un attimo.');
  }else if(guarded){
    /* Non puntiamo subito l'iframe visibile su VixSrc: prima ci assicuriamo
       che il dominio sia gia "caldo" (vedi warmUpDomain sopra), cosi questa
       e sempre almeno la seconda richiesta verso quel dominio in sessione. */
    setFrameMessage(fr,'Caricamento…','Preparazione dello streaming.');
    await ensureDomainWarmedUp(src);
  ...
  try{
    ...
    if(!anime&&isCloudflareGuardedSource(normalizeSourceForDevice(currentSrc,false)))await ensureDomainWarmedUp(normalizeSourceForDevice(currentSrc,false));
    if(seq!==playerStreamSeq||String(currentTvId)!==String(id))return;
    showIframePlayer(fr);
