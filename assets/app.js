...
/* ============================================================
   ...
   ma automatico e quasi impercettibile. Avviene UNA SOLA VOLTA
   per sessione, cosi non interrompe mai le riproduzioni
   successive (che gia oggi funzionano al primo colpo).
   ============================================================ */
...
function scheduleVixsrcSelfHeal(seq,id,type,season,episode,src,startSecs){
  if(cfSelfHealDone)return;
  cfSelfHealDone=true;
  clearTimeout(cfSelfHealTimer);
  cfSelfHealTimer=setTimeout(()=>{
    if(seq!==playerStreamSeq)return; // l'utente ha gia cambiato episodio/contenuto: non serve piu
    if(String(currentTvId)!==String(id))return;
...
async function setPlayerFrameSrc(id,type,season,episode,src,startSecs){
  ...
  if(anime)setFrameMessage(fr,'Caricamento episodio','Un attimo.');
    ...
    if(isCloudflareGuardedSource(src))scheduleVixsrcSelfHeal(seq,id,type,season,episode,src,startSecs);
  }
  try{
